...
Object.defineProperties(obj, {
	...
	color: {
		...
		// 配置不可修改 
		// 一旦设置特性方法， 将无法再次配置writable、 value
		// writable: false
		// value: "orange"
	}
})
...
字符串`[object Object]`本身没有太大的用处，但是通过自定义`toString`方法，可以让对象在自动类型转换时，得到想要的字符串形式。1
...
`join()`方法以指定参数作为分隔符，将所有数组成员连接为一个字符串返回。如果不提供参数，默认用逗号分隔。原数组不变
...
`slice`方法的一个重要应用，是将`类似数组的对象转为真正的数组`。
...
function map(arr, fn) {
	...
	for (var i = 0; i < arr.length; i++) {
		// 执行函数传递参数
		// 需要传递三个参数: 成员值 arr[i]、 索引值 i、 原数组 arr
...
if (!Array.prototype.map) {
	...
	Array.prototype.map = function(fn) {
		...
		for (var i = 0; i < this.length; i++) {
			// 执行函数并传递参数
			// 三个参数: 成员值 this[i] 、 索引值 i、 原数组 this
...
上面代码中，执行到数组的第二个成员时，就会中断执行。`forEach`方法做不到这一点，加 `break`或者 `continue`会报错。注意， `return`也不可打断循环！
...
另一点需要注意的是 `forEach`遍历范围在第一次调用回调之前就已经确定，不会因数组增删而改变。哪怕数组在遍历过程中发生改变，`forEach`也会按照原来的`索引值范围`进行遍历
...
上面的代码中，执行到 `two`的时候删除数组的第一项，本来索引是 `1` 的项因为前面的项删掉了以后，索引就变成 0 了，而 0 已经执行过了，所以并不会再执行一遍
...
上面代码也表示并不会因为遍历过程中增加数组内容而改变。以下内部实现也证明了这一点，在第一次调用回调之前，`arr.length`值已经确定，并没有再次获取。
...
if (!Array.prototype.forEach) {
	...
	Array.prototype.forEach = function(fn) {
		...
		for (var i = 0; i < this.length; i++) {
			// 执行函数并传递参数
			// 需要传递的参数有三个: 成员值 this[i]、 索引值 i、 原数组 this
			fn(this[i], i, this);
		}
	}
}
...
`fill()`方法用来填充数组，用一个固定值修改或者填充一个数组中从起始索引到终止索引内的全部元素，包括终止索引。返回修改或者填充后的数组，注意，此方法会改变原数组。
...
if (!Array.prototype.reduceRight) {
	...
	Array.prototype.reduceRight = function(fn) {
		// 定义当次累积的结果
		// 由于是从倒数第二个成员开始遍历， 所以倒数第一个成员就是当次累积的结果
		var result = this[this.length - 1];
		// 遍历this， 传递参数
		for (var i = this.length - 2; i >= 0; i--) {
			// 执行fn并传递参数
			// 传递四个参数: 上一次的累积结果 result、 当前成员值 this[i]、 当前索引值 i、 原数组
...
上面实现代码证明了为啥 `slice`方法第二个参数不包括自身：`for循环`到最后一位不满足条件，跳出循环了。
...
字符串实例对象的`match`方法对字符串进行正则匹配，返回匹配结果（`一个数组`）。
...
JSON 格式（JavaScript Object Notation 的缩写）是一种用于数据交换的文本格式，它基于JavaScript Programming Language, Standard ECMA-262 3rd Edition - December 1999的一个子集。2001年由 Douglas Crockford 提出，目的是取代繁琐笨重的 XML 格式。
...
`JSON`对象是 JavaScript 的原生对象，用来处理 JSON 格式数据。它有两个静态方法：`JSON.stringify()`和`JSON.parse()`。除了这两个方法, JSON 这个对象本身并没有其他作用，也不能被调用或者作为构造函数调用。

> JSON 是一种数据格式，用来序列化对象、数组、数值、字符串、布尔值和 [`null`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/null) 。它基于 JavaScript 语法，但与之不同：**JavaScript不是JSON，JSON也不是JavaScript**。参考 [JSON：并不是JavaScript 的子集](http://timelessrepo.com/json-isnt-a-javascript-subset)
...
- [JSON格式详解](<https://zhuanlan.zhihu.com/p/43638943>)
...
`需要注意的是：代码上线时最好删除console输出的代码，以免影响性能`。
