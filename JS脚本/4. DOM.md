...
[速查手册](https://wangdoc.com/javascript/dom/index.html)

## 2. DOM 级别

DOM级别（DOM Level） 一共可以分为四个级别：DOM0级、DOM1级、DOM2级和DOM3级。而DOM事件分为3个级别：DOM0级事件处理，DOM2级事件处理和DOM3级事件处理。DOM3级扩展了自定义事件的支持

1级DOM标准并没有定义事件相关的内容，所以没有所谓的1级DOM事件模型。

[详细介绍](https://www.jianshu.com/p/622d994906f7)
...
DOM 的最小组成单位叫做节点（node）。文档的树形结构（DOM 树 、DOM Tree），就是由各种不同类型的节点组成。每个节点可以看作是文档树的一片叶子。
...
[速查手册](https://wangdoc.com/javascript/dom/node.html)
...
[速查手册](https://wangdoc.com/javascript/dom/nodelist.html)

## 3. 扩展操作接口

节点对象除了继承 Node 接口以外，还会继承其他接口。`ParentNode`接口表示当前节点是一个父节点，提供一些处理子节点的方法。`ChildNode`接口表示当前节点是一个子节点，提供一些相关方法。

[速查手册](https://wangdoc.com/javascript/dom/parentnode.html)

# ==通用操作==

## 1. 常用节点类型

节点类型值可使用 `nodeType`方法来检测
...
- `document.getElementsByName(name)`
  ...
  - 根据标签的 `name 属性`获取元素，**可用范围小**
  - 不是所有的元素都有name属性，只有 表单标签 才有。而且某些低版本浏览器兼容有问题。慎用
  - 返回元素对象的数组

- `document.querySelector(css选择器)`
  - 根据 `css选择器`获取元素，**H5新加**，兼容 IE8 及以上，最新的`jquery`等框架都是使用此方法
  - 返回匹配的第一个元素对象
...
querySelectorAll下的CSS选择器是整个 HTML 全局的。它们并不会相对于任何特定的元素，甚至不会相对于调用querySelectorAll的元素，再甚至对调用querySelectorAll的元素的外部元素生效。

参考： [点我1](<https://www.zhangxinxu.com/wordpress/2015/11/know-dom-queryselectorall/>)  [点我2](<https://www.cnblogs.com/dolphinX/p/3354318.html>)

### 节点关系方式

通过 `node`接口提供的节点关系方法来获取，后面没有小括号，都是属性

- `.childNodes`
  - 返回指定元素的子节点集合（数组），包括HTML节点，所有属性，文本节点
- `.children`
  - 返回指定元素的子元素集合（数组）。只包括 HTML 节点，甚至不返回文本节点；继承自 `ParentNode`接口的方法

  - children 在IE（6，7，8）中包含注释节点，注释节点不要写在里面。
- `.firstChild`
  - 返回第一个子节点。如果选定的节点没有子节点，则该属性返回 NULL。
  - IE（6，7，8）中指第一个子元素节点（标签）。在火狐谷歌 IE 9+ 以后都指的是第一个节点（包括空文档和换行节点）。
- `.lastChild`
  - 返回最后一个子节点，其他同上
- `.previousSibling`
  - 返回上一个兄弟节点，其他同上
...
- `.parentNode`
  - 返回自身的父节点
- `.offsetParent`
  - 返回自身定位父元素节点，继承自 `Element`节点的方法
- `节点自身.parentNode.children[index]`
  - 返回任意兄弟节点

### 空白符现象

`高级浏览器`会将`空白折叠现象`形成的空白符当做一个文本节点存在，`IE `会把遇到文本节点左右空白符加上文本自身`会合并`为一个
...
for (var i = 0; i < box_child.length; i ++) {
    console.log(box_child[i], box_child[i].nodeType);
}
```

- 兼容封装：传递一个元素，返回该元素的所有子节点并且去掉空白符
...
- `父节点.appendChild(要插入的新节点)`
  - 给一个元素添加 child 节点
  - 返回参数对象本身
  - append 的意思是追加。如果元素已经有child，则追加到别的child之后。也就是说新添加的 child 进去之后一定是 lastChild
- `父节点.insertBefore(要插入的新节点，参考节点)`
  - 在一个节点前面插入新节点
  - 返回插入的新节点
...
- `被复制的节点.cloneNode(布尔值参数)`
  - 克隆节点
  - 返回克隆的节点
  - 方法内部可以传参，如果括号内是true，深层复制，它还将递归复制当前节点的所有子孙节点。如果是false，只复制节点本身。
...
- `父节点.removeChild(要移除的子节点)`
  - 移除子节点
  - 返回被移除的节点
- `节点名.parentNode.removeChild(node)`
  - 移除自身，自删
  - 没有返回值
- `当前节点.remove()`
  - 移除当前节点。继承自 `ChildNode`接口，使用此方法有个前提，当前节点必须有父节点，不然从接口上继承不了此方法
  - 没有返回值
...
返回元素上指定的属性值（值为字符串类型）。如果指定的属性不存在，则返回  `null`

### 设置属性

`元素名.setAttribute(属性名, 属性值)`

没有返回值；创建或改变某个新属性。如果指定属性已经存在，则只设置该值。如果属性不存在，则创建该属性并设置属性值

### 删除属性

`元素名.removeAttribute(要删除的属性名)`
...
**总结：如果插入的是文本，最好用`textContent`属性代替`innerHTML`。**
...
`Node.textContent` 属性表示一个节点及其后代的文本内容。[MDN详细解释](https://developer.mozilla.org/zh-CN/docs/Web/API/Node/textContent)

Internet Explorer 引入了 `node.innerText`。意图类似（由于 `innerHTML` 不安全），但有以下区别：
...
`Node.innerText` 是一个表示一个节点及其后代的“渲染”文本内容的属性。 这是微软 IE 出的

## 7. 文本操作

注意：文本节点就是字符串内容，包括空字符串，空格等。一般优先使用元素属性的文本操作

### 获取文本

- `套有文本的元素对象.firstChild.substringData(offset，count)`
  - 用于获取字符串，第一个参数为获取开始位置，值为数字，从1开始算；第二个参数为需要获取字符串的长度
  - 返回获取到的字符串，原文本节点不改变

### 插入文本

- `套有文本的元素对象.firstChild.insertData(offset，text)`
  - 在指定位置插入字符串，第一个参数为插入位置，值为数字，从1开始算；第二个参数为插入的字符串。
  - 没有返回值
- `套有文本的元素对象.firstChild.appendData(text)`
  - 在文本节点末端追加字符串
  - 没有返回值

### 替换文本

- `套有文本的元素对象.firstChild.replaceData(offset, count, text)`
  - 用于替换字符串，第一个参数为替换开始位置，值为数字，从1开始算；第二个参数为需要被替换掉的长度，第三个参数为新加入的字符串。
  - 没有返回值

### 分割文本

- `套有文本的元素对象.firstChild.splitText(offset)`
  - 将一个文本节点分成两个文本节点，即按照指定的位置分割`nodeValue`值。原来的文本节点将包含从开始到指定位置之前的内容。这个方法会返回一个新文本节点，包含剩下的文本。splitText()方法返回的节点与原节点的parentNode相同
  - 返回新文本（字符串）

```js
<div id="box">你好呵呵hehe</div>
<script>
    var oBox = document.getElementById('box');
    var newNode = oBox.firstChild.splitText(1);
    console.log(newNode,newNode === oBox.lastChild);// "好呵呵hehe"true
    console.log(oBox.firstChild);//'你'      
</script>
```

### 删除文本

- `套有文本的元素对象.firstChild.deleteData(offset, count)`
  - 用于删除子节点的字符串，第一个参数为删除开始位置，值为数字，从1开始算；第二个参数为需要删除字符串的长度
  - 没有返回值
- `套有文本的元素对象.firstChild.remove(offset, count)`
  - 用于移除当前文本节点
  - 没有返回值

## 6. CSS 操作

注意：要先获取到相关节点（元素对象）才能进行下一步的操作，比如 `css` 样式需要先获取元素对象等

`Element`接口有一些快速获取元素样式属性的操作方法，这些样式属性都是计算后的样式，不局限于样式定义方式

### 获取行内样式

- `元素对象.style`
  - 返回元素行内样式表，是`Object`对象类型，内部包含所有的行内属性和值，不包括内嵌、外链样式

- `元素对象.style.css属性名`
  - 返回属性值。打点调用为属性访问器
- `元素对象.style["属性名"]`
  - 返回属性值，属性访问器`[]`形式必须带双引号或单引号

### 获取计算后的样式

- `window.getComputedStyle(元素对象)`
  - 返回元素计算后的样式表，是`Object`对象类型，内部包含一个元素最终渲染的样式属性，不局限于行内、内嵌、外链区别，只包括权重结果的属性和值。
- `window.getComputedStyle(元素对象).css属性名`
  - 返回属性值。打点调用为属性访问器
- `window.getComputedStyle(元素对象)["属性名"]`
  - 返回属性值，属性访问器`[]`形式必须带双引号或单引号

- `元素对象.currentSytle["属性名"] / .css属性名 `
  - IE（6，7，8）不兼容 getComputedStyle 属性
  - 返回同上
- 兼容方案
...
if(window.getComputedStyle) {
    // 表示高级
    oH2.innerHTML = window.getComputedStyle(oPic).borderWidth
window.getComputedStyle(oPic).getPropertyValue("border-width");
}else {
    // 表示IE6,7,8
    oH2.innerHTML = oPic.currentStyle.borderWidth;
}
```

> 内部样式表中的属性和外部样式表中的属性只能获取不能修改。如果想修改需要通过行间样式表修改，行间样式表的优先级最高，会覆盖内部样式表和外部样式表。

### 设置 css 样式

`css` 样式只能在行内设置修改或者添加

- 直接通过属性访问器来设置元素的 `style` 属性

```
<div id="myDiv">test</div>

var myDiv = document.getElementById("myDiv");
myDiv.style.backgroundColor = "red";   // 打点访问和设置
myDiv.style["backgroundImage"] = "url(...)";  // ["属性名"]访问和设置
```

- 使用 `setAttribute()`方法创建整个样式表，适合一次设置多个样式

```
<div id="myDiv">test</div>

var myDiv = document.getElementById("myDiv");
myDiv.setAttribute("style", "background-color:blue; color:red; border:1px solid black");
```

- 预定一个 `class` 类样式规则，给元素添加类属性
...
var myDiv = document.getElementById("myDiv");
myDiv.setAttribute("class", "money");
```
...
包含范围： content + padding

- 返回元素内边距以内的宽高范围。`number`类型

### 内容区 + 内边距 + 边框

`offsetWidth`  `offsetHeight`

包含范围： content + padding + border

- 返回元素边框以内的宽高范围，也就是实际占有宽高。 `number`类型
...
返回当前视口的高度（即浏览器窗口的高度）。等同于`window.innerHeight`属性减去水平滚动条的高度（如果有的话）
...
[Document 节点](https://wangdoc.com/javascript/dom/document.html#%E5%B1%9E%E6%80%A7)
...
这三个方法用于处理`document`节点的事件。它们都继承自`EventTarget`接口，详细介绍参见《EventTarget 接口》
...
> 当给 HTML 元素标签添加 ID 属性时，会默认在全局中添加此 ID值的属性。这是高级浏览器所支持的，但是这种方式并没有成为标准，建议测试时使用，工作中还是需要通过 `document`正常获取元素。
...
CSS 与 JavaScript 是两个有着明确分工的领域，前者负责页面的视觉效果，后者负责与用户的行为互动。但是，它们毕竟同属网页开发的前端，因此不可避免有着交叉和互相配合。下面介绍如何通过 JavaScript 操作 CSS。
...
/**
	...
	* @json 目标值
	* @time 完成动画的时间
	* @callBack 回调函数 当元素运动完毕之后要执行的函数
	*/

function animate(dom, json, time, callBack) {
	/**
		* 分析:
		* 		我们的编程思路使用定时器和循环，实现动画函数
		* 		既然是要循环动画， 那么每一次动画都是一个步长值
		* 		首要目标就是要得到步长值
		* 		步长值 = 总距离 / 总次数
		* 		总距离 = 目标值 - 初始值
		* 		总次数 = 总时间 / 定时器的间隔时间
...
// 定义一个函数， 通过该函数可以的到元素到页面之间的距离， 返回的是一个对象， 对象中包含元素的left、top值


function offset(dom) {
	/**
		* 思路:
		* 	获取元素的定位父元素，再获取它们之间的距离
		* 	再获取定位父元素的定位父元素， 再获取距离
		* 	再获取定位父元素的定位父元素的定位父元素， 再获取它们之间的距离 直到 => body 再让距离相累加
		*/

	// for (; dom != document.body; dom = dom.offsetParent)
...
glass.onmousedown = function(e) {
	...
	document.onmousemove = function(e) {
		...
		// 改变box1中的背景定位
		box1.style.backgroundPosition = - resultX * 2 + "px " + - resultY * 2 + "px";
	}
}
...
// 当绑定的是mouseenter事件的时候， 在图片和镜片中不断的穿梭，只执行镜片的事件
...
// 当帮顶的是mouseover事件的时候， 在图片和镜片中来回穿梭了，同时触发box和glass的事件



// mouseenter
box.onmouseenter = function() {
	// 当鼠标移入box中的时候，让glass和box1出现
...
// 获取页面卷动值: document.documentElement.scrollTop
...
dot.onmousedown = function(e) {
	...
	var area_height = area.clientHeight;

	// 给doument添加mousemove事件
	document.onmousemove = function(e) {
		// 获取鼠标移动后的位置
		var m_x = e.clientX;
		var m_y = e.clientY;

		var resultX = m_x - x + area_width;
		var resultY = m_y - y + area_height;
...
bindEvent(unit, "mousewheel", function(e) {
	...
	if (direction(e)) {
		// 说明是向下滚动，图片应该从右侧进入，相当于右按钮点击事件
		// 右按钮点击事件的策略: 先拉动， 再瞬移
		idx++;
		// 拉动
		animate(unit, {"left": - width * idx}, 1000, function() {
			// 当猫腻图完全显示之后再去验证信号量
			if (idx > length - 1) {
				// idx 变为0
				idx = 0;
...
// 定义一个函数， 接受一个事件对象， 返回鼠标滚轮的方向
function direction(e) {
	// console.log(e.wheelDelta);
	// 判断e.wheelDelta是够是unfefined，从而确定浏览器
...
var carousel = {
	...
	// width属性
	width: document.getElementById("carousel").clientWidth,
	// 信号量
	idx: 0,
	// 定义锁
	lock: true,
	...
	left: function() {
		...
		var me = this;
		// 拉动
		animate(this.unit, {"left": - this.width * this.idx}, 1000, function() {
			// 当猫腻图完全显示之后，验证信号量
	...
	right: function() {
		...
		if (this.idx < 0) {
			...
			// 瞬移到猫腻图
			this.unit.style.left = - this.width * this.length + "px";
		}
...
function Balloon(img) {
	...
	this.setStyle = function() {
		...
		this.dom.style.backgroundPositionX = -this.positionX * this.width + "px";
		this.dom.style.backgroundPositionY = -this.positionY * this.height + "px";
	}
	// 移动
	this.move = function() {
...
document.onkeydown = function(e) {
	// 获取用户按下的键是哪个
	var key = e.key;
	// 拿着这个key 去数组中与每一个气球的str属性值进行比对 如果比对上了 就让气球从数组中消失并且气球本身从dom上消失
	for(var i = 0; i < arr.length; i++) {
